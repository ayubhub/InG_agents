...
class LinkedInSender:
    ...
    def _find_unipile_user_in_chats(self, linkedin_url: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 502:
                # 502 Bad Gateway - API temporarily unavailable, but don't fail completely
                self.logger.warning(f"Error finding user in chats: {response.status_code} {response.reason} for url: {url}")
                return None
            response.raise_for_status()
    ...
    def check_invitation_status(self, invite_id: str, linkedin_url: str) -> str:
        ...
        try:
            ...
            self.logger.debug(f"Checking invitation status for: {linkedin_url}")
            user_chat = self._find_unipile_user_in_chats(linkedin_url)
            if user_chat:
            ...
            if e.response and e.response.status_code == 502:
                ...
                self.logger.warning(f"API unavailable (502) while checking invitation status for {linkedin_url}: {e}")
                return "pending"  # Don't fail, will retry later
            self.logger.error(f"HTTP error checking invitation status for {linkedin_url}: {e}")
            return "pending"
        except Exception as e:
            self.logger.error(f"Error checking invitation status for {linkedin_url}: {e}")
            return "pending"
